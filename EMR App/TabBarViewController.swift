...
class TabBarViewController: UITabBarController, UITabBarControllerDelegate {
    ...
    func transitionToPCM() { //Handles changes in bluetooth keyboard status
        if (alertController == nil) {
            alertController = UIAlertController(title: "Connection to Bluetooth Keyboard Lost!", message: "Please pair your keyboard and press 'OK'. If you wish to transition to Patient Care Mode, press 'Redirect'.", preferredStyle: .Alert)
            ...
            let redirect = UIAlertAction(title: "Redirect", style: .Default, handler: { (action) -> Void in
                self.performSegueWithIdentifier("showPCM", sender: self) //segue -> PCMVC
                self.alertController = nil //clear alertController for next display cycle
            })
    ...
    func tabBarController(tabBarController: UITabBarController, shouldSelectViewController viewController: UIViewController) -> Bool {//Function called BEFORE the transition takes place
        //Tab Bar controller does NOT have a traditional segue between its tabs, so you must use this & the next delegate protocols to handle passing data between your VCs in the controller.
        return true
    }
    
    func tabBarController(tabBarController: UITabBarController, didSelectViewController viewController: UIViewController) {
        // When the user selects a button to move between views, we will pass the currentPatient & currentUser from the previous VC -> current VC.
